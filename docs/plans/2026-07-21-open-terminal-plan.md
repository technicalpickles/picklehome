...
- Image pinned to `ghcr.io/open-webui/open-terminal:0.11.34` (latest release as of 2026-07-21). Bump deliberately, not via `:latest`. **Note:** the published image tag drops the git release tag's `v` prefix (it comes from `pyproject.toml`'s `version` field via `docker/metadata-action`, not the git tag name) — `ghcr.io/open-webui/open-terminal:v0.11.34` does not exist, only `:0.11.34` does. Confirmed by a failed first deploy; verify any future bump with `docker manifest inspect ghcr.io/open-webui/open-terminal:<tag>` before committing it.
- **`TERMINAL_SERVER_CONNECTIONS` is a `ConfigVar` — env seeding only applies to a brand-new database, never to an already-running instance.** Since `open-webui`'s database here already existed from its original deploy, adding this env var did NOT auto-create the connection; `GET /api/v1/configs/terminal_servers` came back empty after deploy. It had to be added by hand via Admin Settings → Integrations → Open Terminal. Don't expect any future `ConfigVar` added to an already-deployed service to seed automatically — only a truly fresh install gets that for free.
...
In a browser at `https://openwebui.<tailnet>.ts.net`: log in as admin, go to Settings → Integrations, scroll to the **Open Terminal** section (not "Tools"/"External Tools"). **Do not expect it to be seeded automatically if `open-webui` was already deployed before this task ran** — `TERMINAL_SERVER_CONNECTIONS` is a `ConfigVar`, and `ConfigVar` env seeding only applies to a brand-new, user-less database on that container's very first boot. If `open-webui`'s database already existed (i.e. this is adding Open Terminal to an already-running instance, not standing up `open-webui` fresh), the section will show empty, and the connection must be added by hand: click **+**, enter URL `http://open-terminal:8000`, the API key (`Open Terminal` item, `picklehome` vault), Auth Type `Bearer`, Save. Either way, expected end state: a connection named "Open Terminal" with a green "Connected" indicator. Confirm via `docker compose logs open-webui | grep terminal_servers` showing a `POST /api/v1/configs/terminal_servers 200` if you added it manually.
