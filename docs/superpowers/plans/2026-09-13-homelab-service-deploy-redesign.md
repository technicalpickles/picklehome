...
>
> **Also checked (2026-09-14):** Task 4 found that any `docker compose` subcommand run *outside* the `op run` wrapper (its own `build` call) still needs every `${VAR:?required}` var present in the process environment, since compose interpolates at parse time regardless of subcommand — the fix there was placeholder build-time values (see `climate-auto-switch/deploy.sh`). `brineworks-server/deploy.sh` has no unwrapped `docker compose` invocation of its own (confirmed by reading it — the systemd unit's `ExecStart` wraps the entire `up -d --build` in `op run`, so build happens inside the wrapper, not before it), so this task needs no equivalent workaround. Don't assume that holds for every later service — check each one's `deploy.sh` individually (Task 7's revision note does this per-service).
...
>
> **Also carry forward — unwrapped `docker compose` subcommands need placeholder values (found during Task 4, confirmed absent in Task 5).** Compose interpolates `${VAR:?required}` at **parse time**, for *every* subcommand — `build`, `pull`, `exec`, `logs` — not just `up`/`run`. Any `docker compose` invocation in `deploy.sh` that runs *outside* the `op run` wrapper will fail to even parse the compose file once real values stop being available in the process environment, regardless of whether that subcommand actually needs a real secret value. Task 4's fix (`homelab/services/climate-auto-switch/deploy.sh`) was placeholder values scoped to that one invocation, confirmed safe because the Dockerfile has no build-time `ARG`s that read them. Checked for this task's services: **`github-actions-runner/deploy.sh:16`** runs `docker compose -f compose.yaml pull` unwrapped — needs placeholder values for `GITHUB_RUNNER_REPO_URL`/`GITHUB_RUNNER_TOKEN` once those become `environment: ${VAR:?required}` entries. **`woodpecker/deploy.sh:38`** (`pull`) and the health-check loop's `$COMPOSE exec` (`deploy.sh:59-71`) both run unwrapped — same treatment for `WOODPECKER_GITHUB_CLIENT`/`WOODPECKER_GITHUB_SECRET`/`WOODPECKER_AGENT_SECRET`/`WOODPECKER_TS_AUTHKEY`; `exec` into an already-running container doesn't need the placeholder values to match what the container actually started with (that came from the real `op run`-wrapped unit), it only needs the file to parse. `brineworks-agent`, `second-brain-agent`, `taskchampion-sync` have no unwrapped `docker compose` calls in their `deploy.sh` (confirmed by reading each) — no placeholder-values fix needed for those three. `nikke` has no secrets at all (Step 6) — not applicable.
...
- [ ] **Step 6: `nikke`.** No `.env.vars`, no secrets, so no `op run` wrap needed — skip Task 4/5's Steps 1-3 entirely for this one, and nothing else to do here: **Ruling (2026-09-14):** the earlier draft of this step told you to also collapse `deploy-nikke` onto `_deploy-remote` here, contradicting this task's own Files list ("the Justfile collapse onto `_deploy-remote` is Task 9's job, not this task's — don't touch `Justfile` here"). Task 9's own text explicitly already covers `deploy-nikke` in its rewrite list, so nothing is lost by deferring — this step is a no-op for Task 7. Do not touch `Justfile` for `nikke` in this task.
...
- [ ] **Step 1:** Rewrite `deploy-taskchampion`, `deploy-github-runner`, `deploy-woodpecker`, `deploy-nikke`, `deploy-brineworks-agent`, `deploy-second-brain-agent` to each call `just _deploy-remote <service-dir-name>`.

  > **Revised 2026-09-14 — three of the originally-listed recipes are removed from this list; do not touch them.** `deploy-open-webui` was already collapsed onto `_deploy-remote` in Task 8 (verify with `git log -- Justfile` if in doubt — nothing left to do here). `deploy-openclaw` should NOT be collapsed onto `_deploy-remote` at all: unlike every other service, its recipe has required steps `_deploy-remote` doesn't support — an existence check for `openclaw.tools.json5`/`openclaw.mcp.json5`/`openclaw.image.env` (private pickleclaw-repo files, gitignored here, symlinked in), then `scp`-ing those plus `workspace-git-sync.sh` to the host with a `chmod 600` in between, all BEFORE the final `ssh ... git pull && deploy.sh` call. `_deploy-remote` is a single ssh call with no hook point to inject those steps. Collapsing it anyway would silently stop copying those files on every future deploy, breaking the container (it bind-mounts them) with no obvious error at deploy time. `deploy-openclaw` already dropped its `.env` scp step in Task 8 (the actual secrets-migration goal), so it needs no further changes here — leave its custom recipe shape exactly as-is.
  >
  > **`deploy-backup` must NOT be collapsed — this task's file list was wrong to include it.** Verified: `backup`'s systemd unit still uses `EnvironmentFile=/opt/homelab/homelab/services/backup/.env` and `deploy.sh` still `source`s that same file directly for `RESTIC_REPOSITORY`/`RESTIC_PASSWORD` — `backup` was never actually migrated to `op run` by any task in this plan (it runs bare restic under a dedicated system user, not compose/systemd-`ExecStart`-wrapped like every other service, so none of Tasks 4/5/7/8's mechanism applies to it directly). The Global Constraints section's claim that this is handled "(see Task 11)" is wrong — Task 11 is only a README rewrite, it contains no migration work. Collapsing `deploy-backup` onto `_deploy-remote` (which scp's nothing) would silently strip `RESTIC_REPOSITORY`/`RESTIC_PASSWORD` from every future deploy, breaking nightly backups with no loud failure until a restore is needed. This is a real, unaddressed scope gap in this plan (backup needs its own op-run migration — a different shape of problem than the others, since there's no compose file or `ExecStart=`/`ExecStop=` pair to wrap, just a bare script + a systemd `EnvironmentFile=`), not something to invent a fix for inside this mechanical-cleanup task. Leave `deploy-backup` exactly as it is (still scp'ing `.env`, still working) and track the actual migration as a separate follow-up (see this task's own ledger for the taskwarrior reference) — do not touch this recipe.
