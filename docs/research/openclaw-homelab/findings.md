...
| **Resources** | Official doc states only **~2 GB RAM for image *builds*** (1 GB risks OOM/exit 137). **Live-verified: image is 1.53 GB — not ~20 GB** as community guides claimed (likely conflating image size with total disk footprint incl. growing state). **Idle RAM live-verified on the actual J3455 (picklelab, 2026-07-01, via SSH): ~492 MiB, ~1.35% CPU** — comparable to (even slightly better than) the ~573 MiB / ~0.24% CPU measured on Docker Desktop/Mac first. Startup is where the weak hardware shows: **~2.3s cold-start to `[gateway] ready` on picklelab vs. ~243ms on the Mac** (~9–10× slower, expected for a GPU-less Celeron, still fine for an always-on service). The browser tool plugin loads at startup but its actual browser process is lazy-spawned on first use, so idle doesn't pay its cost on either machine — the community "~8 GB with browser" figure is plausibly an active-browsing-task number, not baseline idle. Still unmeasured: RAM/CPU under real agentic load with a working model, and with the browser tool actually invoked. | Image size/idle RAM/startup: **High (live-verified on the real J3455)**; under-load numbers: still open (spike §7) |
...
- **RAM — budget confirmed real, not tight, and now measured on the actual hardware.** Checked live via SSH (2026-07-01): `free -h` shows 15Gi total, **13Gi "available"** (the Linux metric accounting for reclaimable cache, not just "free"), and the existing ~9 services (`obsidian-sync` ×2, `second-brain-agent` ×2, `woodpecker` ×3, `brineworks-*` ×4, `github-actions-runner`, `taskchampion-sync`) sum to **~1 GB RAM total** via `docker stats`, all under 1% CPU. Pulled and ran the real image directly on picklelab (throwaway `--dev` container, removed after, no disruption to the real services): idle footprint **~492 MiB RAM, ~1.35% CPU** — comparable to the Mac's ~573 MiB — but startup took **~2.3s vs. ~243ms** on the Mac, the Celeron showing itself. Even the pessimistic "~8 GB while actively browsing" community figure fits comfortably inside the ~13 GB available. Not a tight squeeze — there's real headroom. What's still unconfirmed is footprint under genuine agentic load with a working model (this test used an unconfigured `--dev` instance).
...
**Confidence:** the persistence claims are source-confirmed (two independent docs agree, same volume architecture picklelab already assumes), and the pull/recreate recipe itself is now **live-tested directly on picklelab (2026-07-01)** — see below. Full detail in `spike-questions.md` §9.

**Live test of the Docker pull/recreate mechanism itself (2026-07-01, directly on picklelab, throwaway instance, cleaned up after):** built a real `compose.yaml` matching this deploy's shape (`image: ${OPENCLAW_IMAGE}`, no `build:` key, real bind mounts), brought it up on `ghcr.io/openclaw/openclaw:2026.6.10`, set a distinctive config marker, then bumped `OPENCLAW_IMAGE` to `:latest` (`2026.6.11`) and ran `docker compose pull && docker compose up -d`. Confirmed: a genuine container **recreate** (not a restart), the config marker survived exactly, `/healthz` returned `200` on the new version, and `openclaw doctor` ran clean post-swap with no migration errors. This is exactly the recipe described above, working as documented, with no `build:`-key ambiguity. Didn't cover: a real deployed instance with actual channel/session state (this used a throwaway dev config), or timing under the J3455's weaker CPU during the swap window specifically.
...
**For the picklelab Docker deploy — resolved (2026-07-01):** not a new repo. The deploy plan now migrates `pickleclaw`'s actual workspace content into `/srv/data/openclaw/workspace/` by cloning its existing private repo (`github.com/technicalpickles/openclaw-workspace`) directly, continuing the same history from the new host rather than starting fresh or relying on restic alone. See [the deploy plan's Migration from pickleclaw section](../../plans/2026-06-30-openclaw-deploy.md#migration-from-pickleclaw).
...
1. **Extra bind-mounted bin dir on PATH — best for spiking arbitrary CLIs like `gogcli`.** Mount a host dir via the official `OPENCLAW_EXTRA_MOUNTS` hook, prepend it to `PATH`. Adding/updating a tool = copy the binary in; the agent's **next shell invocation re-reads PATH**, so no rebuild and no restart. Because the base is **`node:24-bookworm-slim` (Debian/glibc)**, static/Go binaries like `gogcli` run as-is. Keep a committed `tools/` manifest or fetch-script in the repo (binaries themselves stay out of git, fetched on deploy).
   - *Provenance:* the **mount mechanism is now official** (`OPENCLAW_EXTRA_MOUNTS` in the docker doc), and the Debian/glibc base is official (`node:24-bookworm-slim`). **Live-pickup without restart is now confirmed** (2026-07-01, on `pickleclaw`): dropped a new executable into an already-on-`PATH` dir while the gateway service kept running, no restart, and the agent ran it by name on the first ask — see `spike-questions.md` §6. Docker-specific nuance (a `-v` bind mount instead of a native FS write) is unconfirmed but standard bind-mount behavior, not an OpenClaw quirk.
...
- **Tool surface?** Default OpenClaw can run shell + browse + filesystem. Decide the minimum set; disable the browser unless needed (its RAM cost is real but only lands during an active browse task, not idle — still the biggest attack surface to trim).
