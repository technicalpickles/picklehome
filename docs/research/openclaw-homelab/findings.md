...
| **Resources** | Official doc states only **~2 GB RAM for image *builds*** (1 GB risks OOM/exit 137). **Live-verified (2026-07-01, local Docker pull, not the NUC): image is 1.53 GB — not ~20 GB** as community guides claimed (likely conflating image size with total disk footprint incl. growing state). **Idle runtime RAM: ~573 MiB** (not ~4 GB) — the browser tool plugin loads at startup but its actual browser process is lazy-spawned on first use, so idle doesn't pay its cost; the community "~8 GB with browser" figure is plausibly an active-browsing-task number, not baseline idle. Still unmeasured: real footprint on the weak J3455 itself, and under real agentic load with a working model. | Image size/idle RAM: **High (live-verified, off-NUC)**; on-NUC + under-load numbers: still open (spike §7) |
...
- **RAM — budget confirmed real, not tight.** Checked live via SSH (2026-07-01): `free -h` shows 15Gi total, **13Gi "available"** (the Linux metric accounting for reclaimable cache, not just "free"), and the existing ~9 services (`obsidian-sync` ×2, `second-brain-agent` ×2, `woodpecker` ×3, `brineworks-*` ×4, `github-actions-runner`, `taskchampion-sync`) sum to **~1 GB RAM total** via `docker stats`, all under 1% CPU. OpenClaw's own measured idle footprint is ~573 MiB (see Resources row above); even the pessimistic "~8 GB while actively browsing" community figure fits comfortably inside the ~13 GB available. Not a tight squeeze — there's real headroom. What's still unconfirmed is OpenClaw's footprint *on the J3455 itself* (this measurement was on Docker Desktop/Mac hardware) and under genuine agentic load.
