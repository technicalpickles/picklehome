...
Tool profile is `coding` (browser/canvas/automation still explicitly denied — `deny` overrides profile grants) — see `openclaw.tools.json5` in the private `pickleclaw` repo. No `docker.sock` grant; the gateway is containerized and relies on tool policy rather than the in-container Docker sandbox. Full rationale in the design doc's "Security decisions".
