...
Each location is a 1Password item tagged `picklehome-location` with fields: `slug`, `label`, `latitude`, `longitude`, `station_macs` (comma-separated), optional `comfort_mode` (`true` for a location whose thermostat this tooling manages), and optional `yale_houses` (comma-separated Yale house names, consumed by [locks](../locks/README.md) to group locks by location). `just dotenv` discovers every tagged item and snapshots them into the `PICKLEHOME_LOCATIONS` JSON var in `.env` (via `scripts/locations-filter.jq`); the shared `picklehome/locations.py` registry parses it at runtime (climate and locks both consume it). Add or edit an address in 1Password, then re-run `just dotenv` to pick it up.
