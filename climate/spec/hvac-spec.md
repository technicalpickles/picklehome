...
- **Comfort Heat**: primary occupied mode when it's cold out. Target ~70°F by heating. Active when the 24-hour mean outdoor temp is below 60°F (threshold configurable in `climate/config/weather.yaml`).
- **Comfort Cool**: primary occupied mode when it's warm out. Target ~70°F by cooling. Active when the 24-hour mean outdoor temp is above 65°F.
